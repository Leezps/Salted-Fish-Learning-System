		...
		if(ServletFileUpload.isMultipartContent(req)) {
			...
			try {
				...
				if(id == null || phone == null || password == null || code == null || id.equals("") || phone.equals("") || password.equals("") ||code.equals("") || image == null) {
					driverVo = new DriverVO(1000, "数据请求格式有误！", null);
				} else {
					if (code.equals("0")) {
						...
						if(rowNum <= 0) {
							driverVo = new DriverVO(1001, "头像上传失败！", null);
						} else {
							DriverEntity entity = business.getDriverInformation(phone);
							driverVo = new DriverVO(1002, "头像上传成功！", entity);
						}
						...
						if(rowNum <= 0) {
							driverVo = new DriverVO(1003, "身份证图像上传失败！", null);
						} else {
							DriverEntity entity = business.getDriverInformation(phone);
							driverVo = new DriverVO(1004, "身份证图像上传成功！", entity);
						}
						...
						if(rowNum <= 0) {
							driverVo = new DriverVO(1005, "教练证图像上传失败！", null);
						} else {
							DriverEntity entity = business.getDriverInformation(phone);
							driverVo = new DriverVO(1006, "教练证图像上传成功！", entity);
						}
					} else {
						driverVo = new DriverVO(1007, "数据请求代码有误！", null);
					}
				}
