...
public class StudentChangePasswordServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(phone == null || phone.equals("") || phone.length() != 11) {
			...
			studentRegisterVO = new StudentLoginRegisterVO(1001, "手机号不存在！", null);
		} else {
			if(code == null) {
					...
					if(password == null || password.equals("") || password.length() > 20 || confirmPassword == null) {
						...
						if(getMessage.getCode() != null && getMessage.getCode().equals("OK") && getMessage.getSmsSendDetailDTOs().size() > 0) {
							...
							if(message.getOutId().equals(verification)) {
								int rowNums = studentRegister.changeStudentPassword(phone, password);
								if(rowNums <= 0) {
									studentRegisterVO = new StudentLoginRegisterVO(1007, "用户修改未成功！", null);
								} else {
									StudentEntity entity = studentRegister.getStudentInformation(phone);
									studentRegisterVO = new StudentLoginRegisterVO(1008, "用户密码修改成功！", entity);
								}
							} else {
								studentRegisterVO = new StudentLoginRegisterVO(1009, "验证码错误！", null);
							}
						} else {
							studentRegisterVO = new StudentLoginRegisterVO(1010, "验证失败！", null);
						}
					}
				}
			}
